...
MODULE shr_sys_mod
   ...
   !+/- jtb use shr_mpi_mod   ! wraps MPI layer
...
!-------------------------------------------------------------------------------
   ...
   !+\- jtb call shr_mpi_initialized(flag)

   if (flag) then
      if (present(rc)) then
          !+\- jtb call shr_mpi_abort(trim(local_string),rc)
      else
          !+\- jtb call shr_mpi_abort(trim(local_string))
      endif
   endif
